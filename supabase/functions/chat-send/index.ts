...
async function loadContext(
  ...
  // Body is REVOKE'd from authenticated (Phase 7); use admin to read it.
  // Ownership has already been confirmed by the userClient INSERT above.
...
Deno.serve(async (req: Request) => {
  ...
  // Plan gate: AI assistant is paid-only. Reject before inserting the user
  // message so a free user's draft never lands in the table.
  ...
  if (plan === "free") {
    return jsonError(402, "PLAN_LIMIT_CHAT", "AIアシスタントは有料プランで使えます");
  }
  ...
  try {
    ctx = await loadContext(admin, lessonId);
  } catch (err) {
